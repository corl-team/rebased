...
def _ar(
    ...
    kv_idxs = np.apply_along_axis(
        np.random.choice, 1, kv_idxs, replace=False, size=num_queries
    )
    ...
    query_pos = np.apply_along_axis(
        np.random.choice, 1, query_pos_choices, replace=False, size=num_queries
    )
...
def multiquery_ar_in_haystack(
    document_depth: float = 0.,
    vocab_size: int = 8_192,
    num_train_examples: int = 100_000,
    num_test_examples: int = 3_000,
    input_seq_len: int = 64,
    ...
    train_inputs, train_labels = _mqar_in_haystack(
        vocab_size=vocab_size,
        num_examples=num_train_examples,
        input_seq_len=input_seq_len,
        seed=seed,
    ...
    test_inputs, test_labels = _mqar_in_haystack(
        vocab_size=vocab_size,
        num_examples=num_test_examples,
        input_seq_len=input_seq_len,
        seed=seed + 10,  # different seed for test set
...
def _mqar_in_haystack(
    ...
    assert vocab_size > input_seq_len
    assert num_kv_pairs * 4 <= input_seq_len
    assert 0. <= document_depth <= 1., "document_depth must be in [0; 1]"

    np.random.seed(seed)

    # two tokens for key and value
    context_size = num_kv_pairs * 2
    num_tokens_before_context = int((input_seq_len - num_kv_pairs - context_size) * document_depth)
    ...
    before_context = np.zeros((num_examples, num_tokens_before_context), dtype=np.int64)
    after_context = np.zeros((num_examples, input_seq_len - num_tokens_before_context - context_size - num_kv_pairs), dtype=np.int64)

    # queries and answers
    key_permutations = np.stack([np.arange(num_kv_pairs, dtype=int)] * num_examples)
    key_permutations = np.apply_along_axis(
        np.random.permutation, axis=1, arr=key_permutations
    )
    queries = np.zeros((num_examples, num_kv_pairs + 1), dtype=np.int64)
    np.put_along_axis(queries, key_permutations, values=keys, axis=1)
    examples = np.concatenate((before_context, kvs, after_context, queries), axis=1)

    labels = np.full((num_examples, input_seq_len + 1), -100, dtype=np.int64)
    np.put_along_axis(labels, key_permutations + before_context.shape[-1] + context_size + after_context.shape[-1] + 1, values=values, axis=1)
